...
print(f"Fetching Best Buy Deal of the Day (js_render=True): {url}")
...
for i, p in enumerate(products[:10]):
    clean_t = p['title'].encode('ascii', 'ignore').decode('ascii')
    print(f"#{i+1}: {clean_t} | Price: ${p['current_price']} | Brand: {p['brand']}")
